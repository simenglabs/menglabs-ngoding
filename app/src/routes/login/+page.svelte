<script lang="ts">
	...
	async function submit(e: SubmitEvent) {
		...
		try {
			...
			if (!r.ok) { error = d.message ?? 'Login gagal'; return; }
...
<div class="flex min-h-screen items-center justify-center bg-[#0a0f1f] px-4 py-10 text-white">
	<div class="w-full max-w-[400px] rounded-2xl border border-[#1e293b] bg-[#151c2f] p-6">
		...
		<form onsubmit={submit} class="mt-5 space-y-3">
			...
			<input bind:value={password} type="password" placeholder="Password" required class="w-full rounded-xl border border-[#2a3958] bg-[#0f172a] px-3 py-2.5 text-sm placeholder:text-[#475569] focus:border-[#f97316] focus:outline-none" />
			{#if error}<p class="rounded-xl bg-red-950/40 px-3 py-2 text-xs text-red-300">{error}</p>{/if}
			<button type="submit" disabled={loading} class="w-full rounded-xl bg-[#f97316] py-2.5 text-sm font-semibold text-white hover:bg-[#ea6a0f] disabled:opacity-50">{loading ? '...' : 'Login'}</button>
		</form>
		<p class="mt-4 text-center text-xs text-[#64748b]">Belum punya akun? <a href="/register" class="text-[#f97316] hover:underline">Daftar</a></p>
