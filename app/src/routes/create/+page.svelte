<script lang="ts">
	...
	let prdList = $state<{ id: string; title: string; createdAt: string | number }[]>([]);
...
<div
	class="flex min-h-screen flex-col items-center bg-[#121827] px-4 py-16 text-white selection:bg-[#b45309]/30"
>
	<div class="w-full max-w-[720px] text-center">
		<h1
			class="flex items-center justify-center gap-3 text-[34px] leading-none font-extrabold tracking-tight sm:text-[42px]"
		>
			<span>Mau bikin apa?</span>
			<span
				class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-[#1a2a2b] sm:h-11 sm:w-11"
			>
		...
		<p class="mx-auto mt-4 max-w-[560px] text-[15px] leading-relaxed text-[#94a3b8]">
			...
			<span
				class="inline-flex items-center gap-1 rounded-full bg-[#1e2a4a] px-2.5 py-0.5 text-[13px] font-medium text-[#93c5fd]"
			>
				<svg
					width="12"
					height="12"
					...
					fill="none"
					stroke="currentColor"
					stroke-width="2"><path d="M16 18l6-6-6-6" /><path d="M8 6l-6 6 6 6" /></svg
				>
				codebase anda
			</span>
		</p>
	...
	<div class="mt-10 w-full max-w-[640px]">
		<div
			class="rounded-[20px] border border-[#252f47] bg-[#1a2235]/90 p-3 shadow-[0_8px_40px_rgba(0,0,0,0.4)] backdrop-blur sm:p-4"
		>
			<textarea
				bind:value={prompt}
				onkeydown={handleKey}
			...
			<div class="mt-3 flex items-center gap-2">
				<div class="relative">
					...
					{#if showReferensi}
						<div
							class="absolute bottom-full left-0 mb-2 w-64 rounded-xl border border-[#2a3958] bg-[#0f172a] p-3 text-xs text-[#cbd5e1] shadow-xl"
						>
							<p class="font-semibold text-white">Tambah referensi</p>
							<p class="mt-1 text-[#64748b]">
								Upload file / paste link PRD, design, atau repo untuk konteks.
							</p>
							<label class="mt-2 flex items-center gap-2">
								<input type="checkbox" disabled class="rounded border-zinc-600" />
								Referensi file/URL segera hadir
							</label>
						</div>
					{/if}
				</div>
				...
				<div class="relative">
					...
					>
						...
						<svg
							...
							stroke="currentColor"
							stroke-width="2"
							class="opacity-60"><path d="M6 9l6 6 6-6" /></svg
						>
					</button>
					{#if showLang}
						<div
							class="absolute bottom-full left-0 mb-2 w-44 rounded-xl border border-[#2a3958] bg-[#0f172a] py-1 shadow-xl"
						>
							{#each ['Bahasa Indonesia', 'English', '日本語'] as l}
								<button
									...
									class="w-full px-3 py-2 text-left text-xs hover:bg-[#1e293b] {lang === l
										? 'text-white'
										: 'text-[#94a3b8]'}">{l}</button
								>
							{/each}
						</div>
					{/if}
		...
		<button
			...
			class="mx-auto mt-8 flex items-center gap-1.5 text-sm text-[#64748b] hover:text-[#94a3b8]"
		>
			<svg
				width="16"
				height="16"
				...
				fill="none"
				stroke="currentColor"
				stroke-width="1.7"><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></svg
			>
		...
		{#if historyOpen}
			<div
				class="mx-auto mt-4 max-w-[520px] rounded-2xl border border-[#252f47] bg-[#1a2235] p-4 text-left"
			>
				...
				<ul class="mt-2 space-y-2">
					{#each prdList as p}
						<li class="flex items-center justify-between rounded-xl bg-[#0f172a] px-3 py-2 text-xs">
							<button
								onclick={() => goto(`/hasil?id=${p.id}`)}
								class="text-left text-[#cbd5e1] hover:text-white">{p.title}</button
							><span class="text-[#475569]">{new Date(p.createdAt).toLocaleDateString()}</span>
						</li>
					{/each}
				</ul>
			</div>
	...
	<p class="mt-6 text-center text-[11px] text-[#475569]">
		Tekan ⌘+Enter untuk lanjut ke preferensi tech · Draft disimpan lokal
	</p>
</div>
