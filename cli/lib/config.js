...
export function loadConfig() {
  const env = {
    ...
    perencanaanId:
      process.env.MENGLABS_PERENCANAAN || process.env.PERENCANAAN_ID || "",
  };
  let fileCfg = {};
  for (const p of [GLOBAL_FILE, LOCAL_FILE]) {
    try {
      if (fs.existsSync(p))
        fileCfg = { ...fileCfg, ...JSON.parse(fs.readFileSync(p, "utf8")) };
    } catch {}
  }
  return {
    apiBase:
      ...
      fileCfg.API_BASE ||
      "http://localhost:5173",
    apiKey: env.apiKey || fileCfg.apiKey || fileCfg.AGENT_API_KEY || "",
    perencanaanId:
      env.perencanaanId || fileCfg.perencanaanId || fileCfg.perencanaanId || "",
    _file: LOCAL_FILE,
...
export function requireConfig() {
  ...
  if (!c.apiBase)
    throw new Error(
      "apiBase belum set. Jalankan: npx menglabs-ngoding init --url https://your-host --key sk-...",
    );
  return c;
}
